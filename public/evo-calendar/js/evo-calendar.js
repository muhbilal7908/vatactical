...
;(function(factory) {
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        noEventForThisDay: "No Class for this day.. so take a rest! :)",
    ...
    EvoCalendar.prototype.buildEventList = function() {
        ...
        $.ajax({
            url: "/all-slots", // Specify the URL you want to send the request to
            method: "get",
            ...
            success: function(response) {
                ...
                if (response) {
                    for (var i = 0; i < response.length; i++) {
                        if(_.isBetweenDates(_.$active.date, response[i].date)) {
                            eventAdder(response[i])
                        }
                        else if (response[i].everyYear) {
                            var d = new Date(_.$active.date).getMonth() + 1 + ' ' + new Date(_.$active.date).getDate();
                            var dd = new Date(response[i].date).getMonth() + 1 + ' ' + new Date(response[i].date).getDate();
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        markup = `<div class="event-container" data-bs-toggle="modal" data-bs-target="#exampleModal_${event_data.id}" role="button" data-event-index="${event_data.id}">`;
        ...
        // Modal markup
        var modalMarkup = `
            <div class="modal fade" id="exampleModal_${event_data.id}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">${event_data.name}</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                        <div class="w-25 ms-auto d-block">
                           <img class="rounded-circle w-100 shadow" src="/assets/staff/${event_data.staff}">
                           <p class="fw-bold text-center mt-2">${event_data.staff_name}</p>
                           </div>
                           <h2 class="text-capitalize">${event_data.name}</h2>
                           <p>Location:${event_data.location}</p>
                           <p>Date:${event_data.date}</p>
                           <p><span>Start_time:${event_data.start_time}</span><span class="mx-4">End_time:${event_data.end_time}</span></p>
        ...
        // Initialize modal
        var modalSelector = `#exampleModal_${event_data.id}`;
    ...
    EvoCalendar.prototype.buildEventIndicator = function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    _.addEventIndicator(response[i]);
                }
